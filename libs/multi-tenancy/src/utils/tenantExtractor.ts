...
export function extractTenantFromRequest(
  ...
  const { strategy, headerName = 'X-Tenant-ID', queryParam = 'tenant', pathPattern = '/tenant/:slug' } = options;
...
export function isReservedTenantIdentifier(identifier: string): boolean {
  const reserved = [
    ...
    'docs', 'help', 'support', 'blog', 'status', 'security', 'legal',
    'privacy', 'terms', 'about', 'contact', 'pricing', 'features',
    'dashboard', 'console', 'portal', 'panel', 'manager', 'control',
    'system', 'root', 'administrator', 'user', 'users', 'account',
    'billing', 'payment', 'invoice', 'subscription', 'settings',
    'profile', 'preferences', 'notifications', 'alerts', 'reports',
    'analytics', 'metrics', 'logs', 'audit', 'security', 'backup',
    'restore', 'migrate', 'import', 'export', 'download', 'upload'
  ];
