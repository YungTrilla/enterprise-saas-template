    ...
    return (
      <div ref={ref} className={cn('flex h-screen bg-background', className)} {...props}>
        ...
        <div className='flex flex-1 flex-col overflow-hidden'>
          {/* Header */}
          {header && (
            <header className='flex-shrink-0 border-b border-border bg-card'>{header}</header>
          )}
          ...
          {/* Footer */}
          {footer && (
            <footer className='flex-shrink-0 border-t border-border bg-card'>{footer}</footer>
          )}
        </div>
      </div>
    );
...
const Header = React.forwardRef<HTMLElement, HeaderProps>(
  ({ className, title, subtitle, actions, children, ...props }, ref) => (
    <header ref={ref} className={cn('flex items-center justify-between p-6', className)} {...props}>
...
const Sidebar = React.forwardRef<HTMLElement, SidebarProps>(
  ({ className, collapsed = false, children, ...props }, ref) => (
    <nav
      ref={ref}
      className={cn('flex flex-col h-full p-4', collapsed && 'items-center px-2', className)}
      {...props}
    >
      {children}
    </nav>
