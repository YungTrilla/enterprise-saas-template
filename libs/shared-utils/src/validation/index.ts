...
export const commonJoiSchemas = {
  ...
  // Identifiers
  sku: Joi.string().pattern(/^[A-Z0-9-]{3,20}$/).uppercase().trim(),
  slug: Joi.string().pattern(/^[a-z0-9-]+$/).lowercase().trim(),
  ...
  safeText: Joi.string().trim().max(1000).pattern(/^[^<>{}]*$/),
...
export const commonZodSchemas = {
  ...
  // Identifiers
  sku: z.string().regex(/^[A-Z0-9-]{3,20}$/).transform(s => s.toUpperCase()),
  ...
  safeText: z.string().trim().max(1000).regex(/^[^<>{}]*$/, 'Text contains invalid characters'),
 ...
 */
export function validateWithJoi<T>(data: unknown, schema: Joi.Schema): ValidationResult & { data?: T } {
 ...
 */
export function validateWithZod<T>(data: unknown, schema: z.ZodSchema<T>): ValidationResult & { data?: T } {
