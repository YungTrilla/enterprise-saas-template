...
exports.commonJoiSchemas = {
    ...
    password: joi_1.default.string().min(8).max(128).pattern(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]/).message('Password must contain at least one uppercase letter, one lowercase letter, one number, and one special character'),
    // Identifiers
    sku: joi_1.default.string().pattern(/^[A-Z0-9-]{3,20}$/).uppercase().trim(),
    slug: joi_1.default.string().pattern(/^[a-z0-9-]+$/).lowercase().trim(),
    // Dates
    ...
    safeText: joi_1.default.string().trim().max(1000).pattern(/^[^<>{}]*$/),
...
exports.commonZodSchemas = {
    ...
    // Identifiers
    sku: zod_1.z.string().regex(/^[A-Z0-9-]{3,20}$/).transform(s => s.toUpperCase()),
    slug: zod_1.z.string().regex(/^[a-z0-9-]+$/).toLowerCase(),
    // Dates
    ...
    safeText: zod_1.z.string().trim().max(1000).regex(/^[^<>{}]*$/, 'Text contains invalid characters'),
